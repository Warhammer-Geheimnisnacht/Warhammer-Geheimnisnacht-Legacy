...
float4 main( VS_OUTPUT_WATER Input ) : COLOR
{
	...
	refractiveColor = lerp( refractiveColor, waterColor.rgb, 0.8f+(0.7f*vIceFade) );
...
float4 main( VS_OUTPUT_WATER Input ) : COLOR
{
	...
	float2 vBlend = float2( 0.4f, 0.20f );
	
	refractiveColor = lerp( refractiveColor, waterColor.rgb, 0.8f );
	//float3 outColor = refractiveColor * ( 1.0f - fresnel ) + reflectiveColor * fresnel;
	float3 outColor = refractiveColor * ( 1.0f - fresnel ) + reflectiveColor * fresnel;
