...
VS_OUTPUT_BORDER main( const VS_INPUT_BORDER VertexIn )
{
	...
	pos.y = vClampHeight * WATER_HEIGHT + ( 1.0f - vClampHeight ) * pos.y +.1f; // added +.5 which puts borders slightly above ground to prevent clipping
	VertexOut.pos = pos.xyz;

	float4 vDistortedPos = pos - float4( vCamLookAtDir * 0.05f, 0.0f ); // changed from 0.05 to 0.2f to place borders above trees
...
{
	float4 vColor = tex2D( BorderDiffuse, float2( Input.uv.y * 0.1f, Input.uv.x ) ); // add something for desaturation
	...
	vColor.a *= lerp( vColorTint[0].a, vColorTint[3].a, vData.a )*1.15f; //added *1.75f to make borders more prominent
	
	float vPulseFactor = saturate( smoothstep( 0.0f, 0.9f, ( 0.8f - abs( Input.uv.x - 0.5f ) ) + sin( vTime * 2.5f ) * 0.15f ) ) * vSelectionColor.a; 
