...
float3 CalcWaterNormal( float2 uv, float vTimeSpeed )
{
	...
	float vScaleUV = 15.0f; //10.0f - bigger numbers = smaller waves
	float2 time1 = vScaledTime * float2( 0.3f, 0.7f )*0.05f; // added*0.05f
	float2 time2 = -vScaledTime * 0.25f * float2( 0.8f, 0.2f )*0.05f; // 0.75 to 0.25 + added*0.05f
	float2 uv1 = vScaleUV * uv;
	float2 uv2 = vScaleUV * uv * 0.3; // 1.3
	float noiseScale = 12.0f; // 12.0f
...
float4 main( VS_OUTPUT_WATER Input ) : COLOR
{
	...
	float3 refractiveColor = tex2D( WaterRefraction, (refractiveUV.xy - normal.xz * vRefractionScale * 1.0f) ).rgb; // wave pattern size
	...
	float fresnel = saturate( dot( -vEyeDir, normal ) ) * 0.255f; // brightness
	...
	refractiveColor = lerp( refractiveColor, waterColor.rgb, .95f+(0.7f*vIceFade) ); // 0.3f+(0.7f*vIceFade) contrast of waves - higher number = lower contrast
	float3 outColor = refractiveColor * ( 1.25f - fresnel ) + reflectiveColor * fresnel; //brightness
...
float4 main( VS_OUTPUT_WATER Input ) : COLOR
{
	...
	waterHeight /= ( 93.7f / 255.0f );
	waterHeight = saturate( ( waterHeight - 0.995f ) * 50.0f ); // added -25f to increase waterheight
	...
	refractiveColor = lerp( refractiveColor, waterColor.rgb, 0.95f+(0.7f*vIceFade) ); // 0.3f+(0.7f*vIceFade) contrast of waves - higher number = lower contrast
	float3 outColor = refractiveColor * ( 1.25f - fresnel ) + reflectiveColor * fresnel; //brightness
