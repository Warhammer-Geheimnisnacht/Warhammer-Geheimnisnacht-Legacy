...
VS_OUTPUT main( const VS_INPUT_INSTANCE v )
{
	...
	float vHeightScaleFactor = 0.75f + vSummedRandom * 0.8f; // changed from 0.5 to 0.8 to stretch trees to a realistic level
	Out.vPosition = float4( v.vPosition.xyz, 0.0 ); // set to 0 to make trees disappear
...
{
	float3 vColor = GetOverlay( tex2D( DiffuseMap, In.vTexCoord0_TintUV.xy ).rgb, tex2D( TintMap, In.vTexCoord0_TintUV.zw ).rgb, 0.5f )*0.45f; 
	// regulate brightness by changing the last value. default is 1.0f
	...
	return float4( ComposeSpecular( vColor, 0.0f ), 1.0f )*0.5f;
}
