...
static const float3 GREYIFY = float3( 1.0, 0.45, 0.6 ); // 0.212671, 0.715160, 0.072169
...
float4 main( VS_OUTPUT_TERRAIN Input ) : COLOR
{
	...
	float3 vOut = CalculateLighting( sample.rgb, normal )*.6f; // added *.65f to darken terrain mode
...
float4 main( VS_OUTPUT_TERRAIN Input ) : COLOR
{
	...
	float2 vBlend = float2( 0.18f, 0.32f ); // ( 0.25f, 0.3f ) brightness, saturation
	float3 vOut = ( dot(sample.rgb, GREYIFY) * vBlend.x + terrain_color.rgb * vBlend.y );

	vOut = CalculateLighting( vOut, normal )*1.05f; // added *1.05f
