...
static const float3 GREYIFY = float3( 0.45, 0.052, 0.2 ); // vanilla: 0.212671, 0.715160, 0.072169 TOT: 1.0, 0.45, 0.6 
...
float mipmapLevel( float2 uv )
{
    ...
    float d = max( dot(dx, dx), dot(dy, dy) );
    return 0.575f * log2( d ); // increased from 0.5 to 0.575 to eliminate flickering
#endif // PDX_OPENGL
}
...
float4 main( VS_OUTPUT_TERRAIN Input ) : COLOR
{
	...
	float3 normal = normalize( tex2D( HeightNormal, Input.uv2 * vMapSize.zw ).rbg - 0.5f )*0.6; // Underwater terrain - disabled by adding *0.6
	...
	sample.rgb = GetOverlay( sample.rgb, TerrainColor, 0.4f ); //.75f (terrain saturation)
	...
	float3 vOut = CalculateLighting( sample.rgb, normal )*.7f; // added *.7f to darken terrain mode
...
float4 main( VS_OUTPUT_TERRAIN Input ) : COLOR
{
	...
	float3 normal = normalize( tex2D( HeightNormal, Input.uv2 * vMapSize.zw ).rbg - 0.5f )*1.2; //added *1.2 to make the snow transition smoother in colored map modes
	...
	float3 TerrainColor = tex2D( TerrainColorTint, Input.uv2 ).rgb; // Wasteland terrain brightness
	float3 vOut;
	...
	if ( terrain_color.a < 1.0f )
	{
		...
		vOut = sample.rgb;
		float4 vFoWColor = GetFoWColor( Input.prepos, FoWTexture);
		vOut = ApplySnow( vOut, Input.prepos, normal, vFoWColor, FoWDiffuse )*.55f; // Wasteland terrain brightness
	}
	else
	{
		sample.rgb = GetOverlay( sample.rgb, TerrainColor, 0.5f );
		float2 vBlend = float2( 0.2f, 0.32f ); // ( 0.25f, 0.3f ) brightness, saturation
		vOut = ( dot(sample.rgb, GREYIFY) * vBlend.x + terrain_color.rgb * vBlend.y );
	}

	vOut = calculate_secondary( Input.uv, vOut, Input.prepos.xz );
	vOut = CalculateLighting( vOut, normal )*1.45f; // TOT: *1.05f
	vOut = ApplyDistanceFog( vOut, Input.prepos, FoWTexture, FoWDiffuse );
